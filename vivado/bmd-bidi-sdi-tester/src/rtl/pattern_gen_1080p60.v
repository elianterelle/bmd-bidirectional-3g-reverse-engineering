...
module pattern_gen_1080p60(
    ...
    reg [15:0] frame_counter_pattern = 0;
    reg [11:0] x_counter_pattern = 0;
    reg [10:0] y_counter_pattern = 1;

    assign pattern_x = x_counter_pattern;
    assign pattern_y = y_counter_pattern;
    assign pattern_frame = frame_counter_pattern;
    ...
    always @(posedge pixel_pair_clk) begin
        if (reset) begin
            ...
            if (x_counter_pattern >= TOTAL_WIDTH_PX - 2) begin
                x_counter_pattern <= 0;
                
                if (y_counter_pattern >= TOTAL_HEIGHT_PX) begin
                    y_counter_pattern <= 1;
                    frame_counter_pattern <= frame_counter_pattern + 1;
                end else begin
                    y_counter_pattern <= y_counter_pattern + 1;
                end
            end else begin
                x_counter_pattern <= x_counter_pattern + 2;
            end
        end
    end
endmodule
